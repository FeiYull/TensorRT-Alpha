...
EfficientDet::EfficientDet(const utils::InitParameter& param):m_param(param)
{
    ...
    checkRuntime(cudaMalloc(&m_output_classes_device, param.batch_size * 1 * param.topK * sizeof(int)));
...
EfficientDet::~EfficientDet()
{
    ...
    checkRuntime(cudaFree(m_output_scores_device));
    checkRuntime(cudaFree(m_output_classes_device));
