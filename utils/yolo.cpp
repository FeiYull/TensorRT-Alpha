...
yolo::YOLO::YOLO(const utils::InitParameter& param) : m_param(param)
{
    ...
    checkcuda(cudaMalloc(&m_input_src_device,    param.batch_size * 3 * param.src_h * param.src_w * sizeof(unsigned char)));
    ...
    checkcuda(cudaMalloc(&m_input_norm_device,   param.batch_size * 3 * param.dst_h * param.dst_w * sizeof(float)));
    checkcuda(cudaMalloc(&m_input_hwc_device,    param.batch_size * 3 * param.dst_h * param.dst_w * sizeof(float)));
    ...
    checkcuda(cudaMalloc(&m_output_idx_device, m_param.batch_size * m_param.topK * sizeof(int)));
    checkcuda(cudaMalloc(&m_output_conf_device, m_param.batch_size * m_param.topK * sizeof(float)));
...
yolo::YOLO::~YOLO()
{
    ...
    checkcuda(cudaFree(m_output_idx_device));
...
void yolo::YOLO::copy(const std::vector<cv::Mat>& imgsBatch)
{
    ...
    for (size_t i = 0; i < imgsBatch.size(); i++)
    {
        checkcuda(cudaMemcpy(pi, imgsBatch[i].data, sizeof(unsigned char) * 3 * m_param.src_h * m_param.src_w, cudaMemcpyHostToDevice));
        pi += 3 * m_param.src_h * m_param.src_w;
    }
    ...
    for (size_t i = 0; i < imgsBatch.size(); i++)
    {
        checkcuda(cudaMemcpyAsync(pi, imgsBatch[i].data, sizeof(unsigned char) * 3 * m_param.src_h * m_param.src_w, cudaMemcpyHostToDevice, streams[i]));
        pi += 3 * m_param.src_h * m_param.src_w;
    }
...
{
    checkcuda(cudaMemset(m_output_objects_device, 0, sizeof(float) * m_param.batch_size * (1 + 7 * m_param.topK)));
