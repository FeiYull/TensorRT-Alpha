...
PPHunmanSeg::PPHunmanSeg(const utils::InitParameter& param) : m_param(param)
{
    ...
    checkcuda(cudaMalloc(&m_output_mask_device,   m_param.batch_size * 1 * m_param.dst_h * m_param.dst_w * sizeof(float)));
    checkcuda(cudaMalloc(&m_output_resize_device, m_param.batch_size * 1 * m_param.src_h * m_param.src_w * sizeof(float)));
...
PPHunmanSeg::~PPHunmanSeg()
{
    ...
    // output
    checkcuda(cudaFree(m_output_mask_device));
...
bool PPHunmanSeg::init(const std::vector<unsigned char>& trtFile)
{
    ...
    checkcuda(cudaMalloc(&m_output_src_device, m_param.batch_size * m_output_src_area * sizeof(float)));
...
void PPHunmanSeg::postprocess(const std::vector<cv::Mat>& imgsBatch)
{
    ...
    checkcuda(cudaMemcpy(m_output_resize_host, m_output_resize_device, m_param.batch_size * sizeof(float) * m_param.src_w * m_param.src_h, cudaMemcpyDeviceToHost));
}
