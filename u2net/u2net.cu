...
u2net::U2NET::U2NET(const utils::InitParameter& param) : m_param(param)
{
    ...
    checkcuda(cudaMalloc(&m_input_rgb_device,    param.batch_size * 3 * param.src_h * param.src_w * sizeof(float)));
    checkcuda(cudaMalloc(&m_input_resize_device, param.batch_size * 3 * param.dst_h * param.dst_h * sizeof(float)));
    checkcuda(cudaMalloc(&m_input_norm_device,   param.batch_size * 3 * param.dst_h * param.dst_h * sizeof(float)));
    checkcuda(cudaMalloc(&m_input_hwc_device,    param.batch_size * 3 * param.dst_h * param.dst_h * sizeof(float)));
    checkcuda(cudaMalloc(&m_max_val_device,      param.batch_size  * sizeof(float)));
    checkcuda(cudaMalloc(&m_min_val_device,      param.batch_size  * sizeof(float)));
    ...
    checkcuda(cudaMalloc(&m_output_resize_device,    param.batch_size * 1 * param.src_h * param.src_w * sizeof(float)));
...
u2net::U2NET::~U2NET()
{
    ...
    checkcuda(cudaFree(m_input_norm_device));
    checkcuda(cudaFree(m_input_hwc_device));
    checkcuda(cudaFree(m_max_val_device));
    checkcuda(cudaFree(m_min_val_device));

    // output
    checkcuda(cudaFree(m_output_src_device));
    checkcuda(cudaFree(m_output_resize_device));
...
void u2net::U2NET::preprocess(const std::vector<cv::Mat>& imgsBatch)
{
    ...
    for (size_t i = 0; i < imgsBatch.size(); i++)
    {
        ...
        checkcuda(cudaMemcpy(p_max++, max_dev, sizeof(float), cudaMemcpyDeviceToDevice));
...
void u2net::U2NET::postprocess(const std::vector<cv::Mat>& imgsBatch)
{
    ...
    for (size_t i = 0; i < imgsBatch.size(); i++)
    {
        ...
        p_tmp += m_param.dst_h * m_param.dst_w;
        checkcuda(cudaMemcpy(p_min++, min_max_dev.first, sizeof(float), cudaMemcpyDeviceToDevice));
        checkcuda(cudaMemcpy(p_max++, min_max_dev.second, sizeof(float), cudaMemcpyDeviceToDevice));
    }
...
void u2net::U2NET::showMask(const std::vector<cv::Mat>& imgsBatch, const int& cvDelayTime)
{
    ...
    {
        checkcuda(cudaMemcpy(m_output_mask_host, output_mask_device + j * m_param.src_w * m_param.src_h,
