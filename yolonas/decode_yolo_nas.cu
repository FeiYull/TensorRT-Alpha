...
__global__ void decode_yolo_nas_device_kernel(int batch_size, int  num_class, int topK, float conf_thresh,
...
void yolo_nas::decodeDevice(utils::InitParameter param, float* src, int srcWidth, int srcHeight, int srcArea, float* dst, int dstWidth, int dstHeight)
{
	...
	decode_yolo_nas_device_kernel << < grid_size, block_size, 0, nullptr >> > (param.batch_size, param.num_class, param.topK, param.conf_thresh,
