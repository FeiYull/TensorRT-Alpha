...
YOLOv8Seg::~YOLOv8Seg()
{
    ...
    checkcuda(cudaFree(m_output_seg_device));
...
bool YOLOv8Seg::init(const std::vector<unsigned char>& trtFile)
{
    ...
    checkcuda(cudaMalloc(&m_output_seg_device, m_param.batch_size * m_output_seg_area * sizeof(float)));
...
void YOLOv8Seg::postprocess(const std::vector<cv::Mat>& imgsBatch)
{
    ...
    checkcuda(cudaMemcpy(m_output_objects_host, m_output_objects_device, m_param.batch_size * sizeof(float) * m_output_obj_area, cudaMemcpyDeviceToHost));
    checkcuda(cudaMemcpy(m_output_seg_host, m_output_seg_device, m_param.batch_size * sizeof(float) * m_output_seg_area, cudaMemcpyDeviceToHost));
}

void YOLOv8Seg::reset()
{
    checkcuda(cudaMemset(m_output_objects_device, 0, sizeof(float) * m_param.batch_size * m_output_obj_area));
}
