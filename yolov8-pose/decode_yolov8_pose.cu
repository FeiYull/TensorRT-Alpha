...
__global__ void decode_yolov8_pose_device_kernel(int batch_size, int  num_class, int topK, float conf_thresh,
...
void yolov8pose::decodeDevice(utils::InitParameter param, float* src, int srcWidth, int srcHeight, int srcArea, float* dst, int dstWidth, int dstHeight)
{
	...
	decode_yolov8_pose_device_kernel << < grid_size, block_size, 0, nullptr >> > (param.batch_size, param.num_class, param.topK, param.conf_thresh,
...
void yolov8pose::transposeDevice(utils::InitParameter param,
float* src, int srcWidth, int srcHeight, int srcArea, 
